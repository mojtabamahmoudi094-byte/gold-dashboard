...
- "fundPortfolioQuery": نام صندوق (رشته) یا null — وقتی کاربر می‌پرسد پورتفوی/دارایی‌های یک
  صندوق مشخص چیست، یا آن صندوق چه سهمی خریده/فروخته (مثلاً «پورتفوی صندوق آسام چیه؟»،
  «صندوق پایا چی خریده؟»، «کیهان اخیراً چی فروخته؟»)، فقط نام دقیق صندوق را بگذار.
- "fundPortfolioMode": یکی از "holdings"|"buys"|"sells"|null — فقط وقتی fundPortfolioQuery
  ست شده: "holdings" برای «پورتفوی/دارایی‌های صندوق چیه» (پیش‌فرض اگر مشخص نبود)،
  "buys" برای «چی خریده»، "sells" برای «چی فروخته».
...
- اگر topHoldingQuery ست شد، بقیه فیلدهای عددی/بولین/fundReturn*/fundPortfolio* را null بگذار.
- اگر fundReturnFundName ست شد، بقیه فیلدهای عددی/بولین/topHoldingQuery/fundPortfolio* را null بگذار.
- اگر fundPortfolioQuery ست شد، بقیه فیلدهای عددی/بولین/topHoldingQuery/fundReturn* را null بگذار.
...
const GEMINI_SCHEMA = {
  ...
  properties: {
    ...
    fundPortfolioQuery: { type: 'STRING', nullable: true },
    fundPortfolioMode: { type: 'STRING', nullable: true },
  },
  required: [],
}
const OPENROUTER_SCHEMA = {
  ...
  properties: {
    ...
    fundPortfolioQuery: { type: ['string', 'null'] },
    fundPortfolioMode: { type: ['string', 'null'] },
  },
  required: [],
