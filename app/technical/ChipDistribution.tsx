...
export default function ChipDistribution({ symbol, isDark }: Props) {
  ...
  return (
    <section aria-label={`توزیع چیپ ${symbol}`} style={{
      ...
      <p style={{ fontSize: 10.5, color: muted, lineHeight: 1.9, margin: '12px 0 0' }}>
        <strong style={{ color: text }}>راهنما: </strong>
        نمودار مثل یک نردبان قیمتی است؛ بالا گران‌ترین و پایین ارزان‌ترین قیمت معامله‌شده در ۲۱۰ روز اخیر.
        طول هر خط یعنی چه سهمی از کل حجم معاملات در آن قیمت رد شده — خط بلندتر یعنی تراکم بیشتر خریدار در آن
        قیمت. سبز یعنی آن قیمت زیر قیمت فعلی است (حامل سودده)، قرمز یعنی بالای قیمت فعلی (حامل ضررده). خط
        روشن‌رنگ «قیمت فعلی» و خط طلایی «میانگین بهای تمام‌شده» را نشان می‌دهد. قیمتی که تراکم زیادی دارد
        معمولاً حمایت یا مقاومت روانی می‌شود؛ عدد «تمرکز چیپ» پایین‌تر یعنی بیشتر معامله‌گرها در یک بازه
        باریک خریده‌اند. این یک برآورد تقریبی از حجم معاملات است، نه داده واقعی سهامداری روزانه — صرفاً جنبه
        اطلاع‌رسانی دارد و توصیه خرید یا فروش نیست.
      </p>
    </section>
  )
}
