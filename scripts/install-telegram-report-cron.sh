...
#   سهام (تهران ۹:۰۵ تا ۱۲:۳۰ هر ۳۰ دقیقه، بازار ۱۲:۳۰ بسته می‌شود):
#     05:35,06:35,07:35,08:35 و 06:05,07:05,08:05 و 09:00(=۱۲:۳۰ بسته‌شدن) UTC
#   صندوق‌های طلا/نقره/زعفران (تهران ۱۲:۰۰ تا ۱۸:۰۰ هر ۳۰ دقیقه):
#     08:30 و 09:00..14:30 UTC
CRON_TAG="# telegram-report (bourse-sanj)"
CRON_LINES=$(cat <<EOF
$CRON_TAG
35 5,6,7,8 * * 0-3,6 $WRAP stocks >> $LOG 2>&1
5 6,7,8 * * 0-3,6 $WRAP stocks >> $LOG 2>&1
0 9 * * 0-3,6 $WRAP stocks >> $LOG 2>&1
30 8 * * 0-3,6 $WRAP funds >> $LOG 2>&1
0,30 9,10,11,12,13,14 * * 0-3,6 $WRAP funds >> $LOG 2>&1
$CRON_TAG-end
EOF
)
